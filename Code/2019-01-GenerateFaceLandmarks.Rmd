...
avg_female_red <- avg_female
avg_female_red[6:13] <- 0

avg_male_red <- avg_male
avg_male_red[6:13] <- 0

avg_faces <- rbind(avg_female, avg_male, avg_female_red, avg_male_red) * ex

new_means <- matrix(0, nrow = 4, ncol = dim(eigenvec)[1])
for(i in 1:nrow(new_means)){
  new_means[i,] <- t(as.matrix(means))
}

avg_landmarks <- (avg_faces %*% t(as.matrix(eigenvec)) ) + new_means
